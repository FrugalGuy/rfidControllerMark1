...
//#undef RESET
#ifdef RESET
#define resetPin  4      // optional factory reset XXX
...
void setup()
{
  ...
  wiegand26.initReaderOne();  // a second time for good measure
...
void getID()
{
  ...
  readCard = reader1;
  Serial.println();
  Serial.print("reader1Count: "); Serial.print(reader1Count); Serial.print(" "); Serial.println(reader1 & 0xfffffff);
...
boolean isMasterCard(long m) //byte m[])
{
...
void readID(int num)
{
  int start = (num * sizeof(long)) - (sizeof(long)-1);
  storedCard = 0;
  for (int j = 0; j < sizeof(long); j++)
  {
    storedCard = storedCard << 8;
    storedCard |= EEPROM.read(start + j);
  }//for
}//readID
/*
void readID(int num)
...
void eraseID(int i)
{
  ...
  if (i < cnt)
  {
    int start = (cnt * sizeof(long)) - (sizeof(long)-1);
    int finish= (cnt - 1) * sizeof(long);
    byte val;
    for ( ; start <= finish; start++)
    {
      val = EEPROM.read(start + sizeof(long));
...
void addID(long a)
{
  ...
  if ( (cnt < MAX_CARDS) &&
       (-1 == findID(a)) )
  {
    int start = (cnt * sizeof(long)) + 1;
    for (int j = 0; j < sizeof(long); j++)
    {
      EEPROM.write( start + j, a >> sizeof(long)-(j+1) );
    }//for
    EEPROM.write(0, ++cnt);
...
void factoryMode()
{
    ...
    if(reader1Count >= 26)
    {
        if ( 1 ) //(val = Serial.read()) == 2)
        {
            getID();                          // sets 'readCard' and 'checksum'
            if (1) //readCard[5] == checksum)
            {
...
void writeMaster(long m ) //byte m[])
{
    Serial.println(); 
    Serial.print("Writing new Master: "); Serial.println(m, DEC); 
    Serial.print("  0x"); Serial.println(m, HEX);
    for (int i = 0; i < sizeof(long); i++)
    {
        EEPROM.write( i+1, (byte)m >> sizeof(long)-(i+1) );
        Serial.print((byte)m >> sizeof(long)-(i+1), HEX);
//        masterCard[i] = m[i];
    }
    masterCard = m;
